...
class StitchEngine {
    ...
    private fun findPairwiseHomography(
        ...
        if (H != null && !H.empty() && inliers >= profile.minInliers) {
            ...
            H.release()
            val translationFallback = if (profile.allowTranslationFallback) {
                buildTranslationFallback(kpA, kpB, good, profile)
            } else {
                null
            }
    ...
    private fun runStitch(frames: List<Mat>, label: String, profile: StitchProfile): StitchResult {
        ...
        try {
            ...
            for (i in 1 until frames.size) {
                ...
                val pairResult = findPairwiseHomography(
                    frames[i],
                    features[i]!!,
                    frames[i - 1],
                    features[i - 1]!!,
    ...
    private fun getImplausibleTransformReason(
        ...
        val sourceBounds = computeWarpedBounds(transform, sourceFrame.width().toDouble(), sourceFrame.height().toDouble())
        ...
        val widthLimit = maxOf(sourceFrame.width(), referenceFrame.width()).toDouble() * profile.maxPairWidthMultiplier
        val heightLimit = maxOf(sourceFrame.height(), referenceFrame.height()).toDouble() * profile.maxPairHeightMultiplier
        val areaLimit = maxOf(
