...
/**
 * ImageAnalysis.Analyzer orchestrator: pulls luma Mat from each frame,
 * runs blur + motion + reads tilt + luma-mean, aggregates CoachingSignals,
 * and emits via [EventEmitter] throttled to 10 Hz.
 *
 * When a [PanoramaSession] is active, evaluates keyframe acceptance
 * via [KeyframeDecider] and triggers incremental stitching.
 */
class FrameAnalyzer(
    ...
    override fun analyze(image: ImageProxy) {
        try {
            ...
            if (activeSession != null && !activeSession.isCancelled) {
                evaluateKeyframe(activeSession, blurScore, motionMagnitude, tiltDeg, overlapPct, lumaMean, now, mat)
            } else {
                mat.release()
            }
        } finally {
    ...
    /**
     * Evaluates whether the current frame qualifies as a keyframe.
     * If accepted: saves to disk, emits `keyframeAccepted`, triggers incremental stitch.
     */
    private fun evaluateKeyframe(
        session: PanoramaSession,
        blur: Float, motion: Float, tilt: Float,
        overlap: Float, luma: Float, nowMs: Long, lumaMat: Mat,
    ) {
        val accepted = session.keyframeDecider.evaluate(
        ...
        // Convert luma → BGR for storage and stitching
        val bgrMat = Mat()
        ...
        val qualityScore = (blur * 0.4f + (1f - motion) * 0.3f + (1f - tilt / 90f).coerceIn(0f, 1f) * 0.3f)
            .coerceIn(0f, 1f)
                ...
                try {
                    ...
                    if (result.success && result.panorama != null) {
                        ...
                        emitter.emit("stitchProgress", JSObject().apply {
                            ...
                            put("totalCells", session.acceptedFrames.size) // inferred at commit
